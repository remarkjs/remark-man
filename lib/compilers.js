 ...
 * @return {string} - Strongly emphasised node.
 */
function bold(node) {
 ...
 * @return {string} - Roff reference.
 */
function reference(node) {
    var url = this.links[node.identifier.toUpperCase()];

    return this.visitors.link.call(this, node, url);
}
 ...
 *
 * @example
 *   text(node) // 'foo \[em] bar'
 *
 * @param {Node} node - Text node.
 * @return {string} - Escaped roff text.
 */
function text(node) {
...
function list(node) {
    ...
    while (++index < length) {
        ...
        values.push(fn.call(self, children[index], bullet, index));
    }
...
function root(node) {
    ...
    visit(node, 'heading', function (definition) {
        ...
        /* istanbul ignore else - legacy remark-slug */
        if ('id' in data) {
            id = data.id;
        } else {
